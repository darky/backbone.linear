(function() {
  ...
  var proxy = Backbone.LinearModel.extend();
  var klass = Backbone.Collection.extend({
    url : function() { return '/collection'; }
  });
  var doc, collection;

  module("Backbone.LinearModel", {
  ...
  test("initialize", 3, function() {
    var Model = Backbone.LinearModel.extend({
      initialize: function() {
        this.one = 1;
  ...
  test("underscore methods", 5, function() {
    var model = new Backbone.LinearModel({ 'foo': 'a', 'bar': 'b', 'baz': 'c' });
  ...
  test("chain", function() {
    var model = new Backbone.LinearModel({ a: 0, b: 1, c: 2 });
    deepEqual(model.chain().pick("a", "b", "c").values().compact().value(), [1, 2]);
  });

  test("clone", 10, function() {
    var a = new Backbone.LinearModel({ 'foo': 1, 'bar': 2, 'baz': 3});
    var b = a.clone();
    equal(a.get('foo'), 1);
    equal(a.get('bar'), 2);
    equal(a.get('baz'), 3);
    ...
    var foo = new Backbone.LinearModel({p: 1});
    var bar = new Backbone.LinearModel({p: 2});
  ...
  test("isNew", 6, function() {
    ...
    a = new Backbone.LinearModel({ 'foo': 1, 'bar': 2, 'baz': 3, 'id': -5 });
    ok(!a.isNew(), "any defined ID is legal, negative or positive");
    a = new Backbone.LinearModel({ 'foo': 1, 'bar': 2, 'baz': 3, 'id': 0 });
    ok(!a.isNew(), "any defined ID is legal, including zero");
    ok( new Backbone.LinearModel({          }).isNew(), "is true when there is no id");
    ok(!new Backbone.LinearModel({ 'id': 2  }).isNew(), "is false for a positive integer");
    ok(!new Backbone.LinearModel({ 'id': -5 }).isNew(), "is false for a negative integer");
  });
  ...
  test("has", 10, function() {
    var model = new Backbone.LinearModel();
  ...
  test("matches with predicate", function() {
    var model = new Backbone.LinearModel({a: 0});
  ...
  test("set and unset", 8, function() {
    var a = new Backbone.LinearModel({id: 'id', foo: 1, bar: 2, baz: 3});
    var changeCount = 0;
  ...
  test("set falsy values in the correct order", 2, function() {
    var model = new Backbone.LinearModel({result: 'result'});
  ...
  test("multiple unsets", 1, function() {
    ...
    var model = new Backbone.LinearModel({a: 1});
  ...
  test("using a non-default id attribute.", 5, function() {
    var MongoModel = Backbone.LinearModel.extend({idAttribute : '_id'});
  ...
  test("set an empty string", 1, function() {
    var model = new Backbone.LinearModel({name : "Model"});
    model.set({name : ''});
  ...
  test("clear", 3, function() {
    var changed;
    var model = new Backbone.LinearModel({id: 1, name : "Model"});
  ...
  test("defaults", 4, function() {
    var Defaulted = Backbone.LinearModel.extend({
      defaults: {
        "one": 1,
        "two": 2
      }
    ...
    Defaulted = Backbone.LinearModel.extend({
      defaults: function() {
        return {
          "one": 3,
          "two": 4
  ...
  test("change, hasChanged, changedAttributes, previous, previousAttributes", 9, function() {
    var model = new Backbone.LinearModel({name: "Tim", age: 10});
  ...
  test("changedAttributes", 3, function() {
    var model = new Backbone.LinearModel({a: 'a', b: 'b'});
    deepEqual(model.changedAttributes(), false);
  ...
  test("change with options", 2, function() {
    var value;
    var model = new Backbone.LinearModel({name: 'Rob'});
  ...
  test("change after initialize", 1, function () {
    ...
    var obj = new Backbone.LinearModel(attrs);
  ...
  test("save within change event", 1, function () {
    var env = this;
    var model = new Backbone.LinearModel({firstName : "Taylor", lastName: "Swift"});
    model.url = '/test';
    model.on('change', function () {
      model.save();
  ...
  test("validate after save", 2, function() {
    var lastError, model = new Backbone.LinearModel();
  ...
  test("save will pass extra options to success callback", 1, function () {
    var SpecialSyncModel = Backbone.LinearModel.extend({
      sync: function (method, model, options) {
        _.extend(options, { specialSync: true });
        return Backbone.LinearModel.prototype.sync.call(this, method, model, options);
      },
      urlRoot: '/test'
    });
  ...
  test("destroy", 3, function() {
    ...
    var newModel = new Backbone.LinearModel;
  ...
  test("validate on unset and clear", 6, function() {
    var error;
    var model = new Backbone.LinearModel({name: "One"});
    model.validate = function(attrs) {
      if (!attrs.name) {
        error = true;
        return "No thanks.";
  ...
  test("Inherit class properties", 6, function() {
    var Parent = Backbone.LinearModel.extend({
  ...
  test("hasChanged/set should use same comparison", 2, function() {
    var changed = 0, model = new Backbone.LinearModel({a: null});
    model.on('change', function() {
  ...
  test("set value regardless of equality/change", 1, function() {
    var model = new Backbone.LinearModel({x: []});
    var a = [];
    model.set({x: a});
    ok(model.get('x') === a);
  });
  ...
  test("set same value does not trigger change", 0, function() {
    var model = new Backbone.LinearModel({x: 1});
  ...
  test("unset does not fire a change for undefined attributes", 0, function() {
    var model = new Backbone.LinearModel({x: undefined});
  ...
  test("`previous` for falsey keys", 2, function() {
    var model = new Backbone.LinearModel({0: true, '': true});
  ...
  test("`save` with `wait` sends correct attributes", 5, function() {
    var changed = 0;
    var model = new Backbone.LinearModel({x: 1, y: 2});
    model.url = '/test';
  ...
  test("#1412 - Trigger 'sync' event.", 3, function() {
    var model = new Backbone.LinearModel({id: 1});
  ...
  test("#1943 change calculations should use _.isEqual", function() {
    var model = new Backbone.LinearModel({a: {key: 'value'}});
  ...
  test("isValid", function() {
    var model = new Backbone.LinearModel({valid: true});
    model.validate = function(attrs) {
